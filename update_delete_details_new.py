...
class UpdateDeletePatientPage(ctk.CTkFrame):
    ...
    def create_widgets(self):
        # -------- Header --------
        header = ctk.CTkFrame(self, fg_color="#D0D0D0", height=80)
        header.pack(fill="x")

        self.header_username_label = ctk.CTkLabel(header, text=f"Username: {self.logged_in_user}",
                                                  font=("Calibri", 14), text_color="black")
        self.header_username_label.place(x=20, y=10)

        ctk.CTkLabel(header, text=f"Date: {self.login_date}",
                     font=("Calibri", 14), text_color="black").place(x=20, y=35)

        ctk.CTkLabel(header, text="MADHAVI DIAGNOSTIC CENTER",
                     font=("Calibri", 24, "bold"), text_color="black").place(relx=0.5, y=20, anchor="center")
        
        self.time_label = ctk.CTkLabel(header, text="", font=("Calibri", 14), text_color="black")
        ...
        try:
            ...
            logout_button = ctk.CTkButton(header, image=logout_icon, text="",
        ...
        ctk.CTkButton(self, text="Clear", command=self.clear_fields,
                      width=80, height=30, fg_color="#D9D9D9", text_color="black", corner_radius=50).place(x=800, y=100)
        ...
        ctk.CTkButton(self, text="Update", command=self.update_patient,
                      width=100, height=35, fg_color="#D0D0D0", text_color="black", hover_color="#C0C0C0", corner_radius=50).place(x=400, y=580)

        ctk.CTkButton(self, text="Delete", command=self.delete_patient,
                      width=100, height=35, fg_color="#D0D0D0", text_color="black", hover_color="#C0C0C0", corner_radius=50).place(x=550, y=580)

        ctk.CTkButton(self, text="Back", command=self.back_callback,
                      width=100, height=35, fg_color="#D0D0D0", text_color="black", hover_color="#C0C0C0", corner_radius=50).place(x=100, y=600)
        ...
        if keyword:
            ...
            if results:
                for pid, fname, lname in results:
                    btn = ctk.CTkButton(self.scrollable_inner, text=f"{fname} {lname}", text_color="black",
                                        fg_color="#E0F7FA", hover_color="#B2EBF2", width=380, anchor="w",
    ...
    def delete_patient(self):
        ...
        if confirm:
            try:
                conn = connect_db()
                cursor = conn.cursor()
                cursor.execute("DELETE FROM patient_details WHERE patient_id = %s", (self.selected_patient_id,))
                conn.commit()
                conn.close()
